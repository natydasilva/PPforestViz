...
\begin{figure}[hbpt]
\centering
<<trcomb, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.width=7, results='hide', error=FALSE>>=
...
\begin{figure}[hbpt]
\centering
<<treenode, echo=FALSE, warning=FALSE, message=FALSE,fig.height=12,fig.width=12>>=
...
p <- ggplot(filter(aux, !ids %in% tree),
            aes(x = Variables , y = Abs.importance , group = ids)) + geom_jitter(height = 0, size = I(4),
                                                                                 alpha = 0.2) +
...
d_boundary <- function(node){
  ...
  p1 <- ggplot(data = dat) + geom_point(ggplot2::aes_string(y = (colnames(dat_2d(node ))[[2]]),
                                        ...
                                        alpha = .5, size = I(4)) + xlim(c(min(dat[,2],dat[,3]),max(dat[,2],dat[,3]))) + ylim(c(min(dat[,2],dat[,3]),max(dat[,2],dat[,3]) ))+
...
\begin{figure}[hbpt]
\centering
<<ternary, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.width=7.5>>=
...
##ternary plot
ternaryshell <- function(ppf, sp = 3, dx = 1, dy = 2, v1=1, v2=2, v3=3, leg=FALSE){
  
  s <- simplex(sp)
  pts <- data.frame(s$points)
  #pts <- data.frame(s$points[,c(dx,dy)])
  gg1 <- ternarydata(ppf, v1, v2, v3)
  levels(gg1$Class) <- c("BF", "BM", "OF", "OM")
...
t1 <- ternaryshell(ppf, 3, 1, 2,leg=TRUE)
t2 <- ternaryshell(ppf, 3, 1, 3, leg=TRUE)
t3 <- ternaryshell(ppf, 3, 2, 3,leg=TRUE)


t1+t2+t3+plot_layout(guides = "collect") &
...
side <-  function(ppf, ang = 0, lege = "bottom", siz = 2.5,
  ...
  levels(voteinf$Type) <- c("BF", "BM", "OF", "OM")
  levels(voteinf$pred) <- c("BF", "BM", "OF", "OM")
  voteinf$Class <- as.factor(voteinf$Class)
  levels(voteinf$Class) <- c("BF", "BM", "OF", "OM")
...
\begin{figure}[hbpt]
\centering
\includegraphics[width=1.05\linewidth]{figure/fish11.png}
...
\begin{figure}[hbpt]
\centering
\includegraphics[width=1.1\linewidth]{figure/tab1com2.png}
...
\begin{figure}[hbpt]
\centering
\includegraphics[width=1.1\linewidth]{figure/tab2comp.png}
...
\begin{figure}[hbpt]
\centering
\includegraphics[width=1.05\linewidth]{figure/fishT2.pdf}
...
\begin{figure}[hbpt]
\includegraphics[width=1\linewidth]{figure/fish31.png}
