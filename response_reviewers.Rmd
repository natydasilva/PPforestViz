...
However, for this potential to materialize the authors should put more emphasis on the limitations of their approach. In particular, the app focuses on the first three nodes of the trees in question, which would be much less informative in settings requiring deep trees. \textcolor{violet}{The app was modified to adress this issue. Now the user can select specific nodes to see (three or more nodes). If there are a lot of nodes will be better to select 4 or less each time to see better the complete display.}
...
\textcolor{violet}{Random Forest presents two sources of randomness in the tree: random variable selection and random bootstrap samples. The random variables selection used to node partition, each split are identified from a random subset of all variables. There is not an initial sample of variables for each tree, the sample is for each tree in each  node partition, in each case using all the variables.
We did not explore formally the sensitivity of the random variable sample size in the importance variable measure. In the algorithm you can select the proportion of variables used in each node partition. In the examples presented in the paper for crab and fishcatch data we use 2 variables in each node partition.
We have included an image from the original paper with the boundary that would result from a classification tree fitted using the PPtree algorithm to take into account your last request. The plot shows a simple example from simulated data with two features and three classes. The partitions generated by PPtree algorithm are oblique to the axis, incorporating the association between the two variables.
}
...
\textcolor{violet}{As you mentioned the data should be standardized for the coefficients to be interpretable as importances, we fixed the density plot and the x axis are in standardized range now.}


Is the distribution being plotted here the value of the linear combination for each class, rather than a single variable? 

\textcolor{violet}{The density plots shows the data projection at each node. }
...
\textcolor{violet}{We have changed the example but even in the previous one in the first node partition the separation was not perfect as you mentioned. We explore your suggestion (changing the bound for the KDE to observe range) but the result was not better and we keep the original.} 
...
\textcolor{violet}{We have complete control over the color palette and the selected palette (Dark2) is a qualitative palette which is suited to represent categorical data and is also colorblind friendly. In this case we are not changing the color palette as you suggested  because maybe for the density plot your idea will work but in the other visualizations using different shades to differentiate sexes will not be easy to identify when the geometric element for example are points. Also in terms of perception using different colors is better than using shapes to differentiate classes.} 
...
\textcolor{violet}{The numbers  in the gray boxes are the predicted class, we fixed the plot to show the class names}
...
\textcolor{violet}{Yes, the coefficient values are between -1 and 1 by construction. We find the linear combinations, \textbf{a} which maximize $\frac{a^TBa}{a^TWa}$, where B y W are the between and within group sums of squares. We get $\hat a_1=e_1$ where $e_1...e_s$ are eigenvectors of $W^{-1}B$. In this case we find the 1-D projection optimizing the LDA index. We have included in Section 3 LDA and PDA indexes which are optimized to get the linear projections and it is more clear now how the coefficients are computed. }
...
\textcolor{violet}{Formulas for all VI were defined in equations and mentionedd in the text. It  is more clear now.}
...
\textcolor{violet}{In Figure 4 the dimension of the simplex is G-1, since in crab data there are 4 clases in this case we work with a three dimensional simplex. Ternary plot can be generalized to more than three dimensional space, with G classes, the ternary plot idea is generalized to a G-1 dimensional simplex. Then if we have 4 classes the generalized ternary plot is a tetrahedron as shown in Figure 4 but if we have more than four classes we can also generalize the ternary plot idea. In the fishcatch data used in this paper to illustrate the shiny app there are 7 classes and we generalized the ternary plot to a 6-dimensional simplex as it is shown in the last panel of Figure 6 (6-didmensional simplex shown pairwise), using tourr package you can see better the 6-dimensional simplex in different rotations https://vimeo.com/707047875. The structure is complex because a 6-dimensional simplex has 7 vertices, 21 eddges and 35 faces.} 
...
\textcolor{violet}{NEED HELP, not sure how to improve this sentence and I do not know what is more descriptive for widget. At their core Shiny widgets are mini-applications created using the shinyApp function.}
...
\textcolor{violet}{Yes fishcatch data are more difficult to classify and then more interesting to show some of the results. For simplicity to describe the main plot of the app we use crab data but to describe the app we prefer to present a more complex problem, more clases, umbalanced data...}
...
\textcolor{violet}{PDP can be order in different ways, we have modified the PDP and included variable order based on the mean value. As you suggested, we have also change the grid lines and just keep the vertical grid in gray, it is more clear now.  
In the MDS we just selected two dimensions but can be modify if it is needed by the problem.
...
For the bottom two plots (variable importance plots) we have fixed the axis and  include titles in each plot, now is easy to compare them.}
